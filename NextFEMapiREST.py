...
class NextFEMrest:
    ...
    def addEdgeLoad(self, elem, values, edge, direction, loadcase, local): return bool(self.nfrest('POST', '/load/element/edgeadd/'+elem+'/'+json.dumps(values)+'/'+str(edge)+'/'+str(direction)+'/'+loadcase+'/'+str(local)+'', values, None))
    ...
    def addObject(self, o, other): return bool(self.nfrest('POST', '/model/addobject/'+str(o)+'/'+str(other)+'', o, None))
    ...
    def addOrModifyCustomData(self, key, value): return bool(self.nfrest('POST', '/model/customdata'+key+'/'+value+'', value, None))
    ...
    def addSpringNLProperty(self, name, NLdofs, NLprops, local): return bool(self.nfrest('POST', '/springproperty/nl/add/'+name+'/'+json.dumps(NLdofs)+'/'+json.dumps(NLprops)+'/'+str(local)+'', NLprops, None))
    ...
    def addSpringsOnOverlappedNodes(self, n, propName): return json.loads(self.nfrest('POST', '/element/add/springsonnodes/'+json.dumps(n)+'/'+propName+'', n, None))
    ...
    def addSubsoilNodalSpringsOnElements(self, n, propName): return bool(self.nfrest('POST', '/element/add/soilsprings/'+json.dumps(n)+'/'+propName+'', n, None))
    ...
    def addSurfaceLoad(self, elem, values, direction, loadcase, local): return bool(self.nfrest('POST', '/load/element/surfaceadd/'+elem+'/'+json.dumps(values)+'/'+str(direction)+'/'+loadcase+'/'+str(local)+'', values, None))
    def addThermalDistLoad(self, elem, values, loadcase): return bool(self.nfrest('POST', '/load/element/tempdistadd/'+elem+'/'+json.dumps(values)+'/'+loadcase+'', values, None))
    ...
    def appendDocXformula(self, TeXcode, alignment): return bool(self.nfrest('POST', '/op/docx/appendformula/'+TeXcode+'/'+str(alignment)+'', TeXcode, None))
    ...
    def appendDocXtext(self, text, alignment): return bool(self.nfrest('POST', '/op/docx/appendtext/'+json.dumps(text)+'/'+str(alignment)+'', text, None))
    def applyButterworthFilter(self, values, samplingF, cutF, order, lowPass): return json.loads(self.nfrest('POST', '/op/bwfilter/'+json.dumps(values)+'/'+str(samplingF)+'/'+str(cutF)+'/'+str(order)+'/'+str(lowPass)+'', values, None))
    ...
    def changeSpringNLProperty(self, name, NLdofs, NLprops): return bool(self.nfrest('POST', '/springproperty/nl/change/'+name+'/'+json.dumps(NLdofs)+'/'+json.dumps(NLprops)+'', NLprops, None))
    def changeSpringNLPropertyDof(self, name, DoF, NLtype, NLprops): return bool(self.nfrest('POST', '/springproperty/nl/change/'+name+'/'+str(DoF)+'/'+str(NLtype)+'/'+json.dumps(NLprops)+'', NLprops, None))
    ...
    def getAreaByNodes(self, nodes): return float(self.nfrest('POST', '/node/area'+json.dumps(nodes)+'', nodes, None))
    ...
    def getMaxMinNodeDispl(self, dir, nodes): return json.loads(self.nfrest('POST', '/res/maxmindispl/'+str(dir)+'/'+json.dumps(nodes)+'', nodes, None))
    ...
    def importDXF(self, stream): return bool(self.nfrest('POST', '/op/import/dxfstream'+str(stream)+'', stream, None))
    ...
    def importMidas(self, model): return bool(self.nfrest('POST', '/op/import/midastext'+json.dumps(model)+'', model, None))
    def importMidasResults(self, path): return bool(self.nfrest('GET', '/op/import/midasresult', None, dict([("path",path)])))
    def importMidasResults(self, text): return bool(self.nfrest('POST', '/op/import/midasresulttext'+json.dumps(text)+'', text, None))
    ...
    def importSismicadSects_Combo(self, text): return bool(self.nfrest('POST', '/op/import/sismicadsettext'+json.dumps(text)+'', text, None))
    ...
    def importStrausResults(self, text): return bool(self.nfrest('POST', '/op/import/straus7resulttext'+json.dumps(text)+'', text, None))
    ...
    def LangTrasl(self, input): return self.nfrest('POST', '/op/trasl'+input+'', input, None)
    ...
    def setLoadA(self, load): return bool(self.nfrest('POST', '/load/setA'+json.dumps(load)+'', load, None))
    ...
    def setNodeCoordinates(self, ID, coords): return bool(self.nfrest('POST', '/node/ID'+ID+'/'+json.dumps(coords)+'', coords, None))
    ...
    def setNodePosition(self, node): return bool(self.nfrest('POST', '/node/ID'+str(node)+'', node, None))
